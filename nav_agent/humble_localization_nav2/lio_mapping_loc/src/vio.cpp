...
void VIOManager::initializeVIO() {
    ...
    for (int grid_row = 1; grid_row <= grid_n_height; grid_row++) {
      for (int grid_col = 1; grid_col <= grid_n_width; grid_col++) {
        ...
        for (float d_temp = d_min; d_temp <= d_max; d_temp += step) {
          ...
          // xyz[2] = d_temp;
          SamplePointsEachGrid.emplace_back(xyz);
        }
        rays_with_sample_points.emplace_back(SamplePointsEachGrid);
      }
    }
  ...
  if (colmap_output_en) {
    ...
    fout_tum.open(DEBUG_FILE_DIR("Colmap/sparse/0/poses.txt"), ios::out);
...
void VIOManager::insertPointIntoVoxelMapLRU(VisualPoint *pt_new) {
  ...
  VOXEL_LOCATION position((int64_t)loc_xyz[0], (int64_t)loc_xyz[1],
                          (int64_t)loc_xyz[2]);
  auto iter = visual_feat_map_.find(position);
  if (iter != visual_feat_map_.end()) {
    iter->second->second->voxel_points.emplace_back(pt_new);
    iter->second->second->count++;
    // 更新的放至最前
    feat_map_cache_.splice(feat_map_cache_.begin(), feat_map_cache_,
                           iter->second);
    iter->second = feat_map_cache_.begin();
  } else {
    VOXEL_POINTS *ot = new VOXEL_POINTS(0);
    ot->voxel_points.emplace_back(pt_new);
    feat_map_cache_.push_front({position, {ot}});
    visual_feat_map_.insert({position, feat_map_cache_.begin()});

    // LRU
    if (feat_map_cache_.size() >= max_voxel_num_) {
      // 删除一个尾部的数据
      visual_feat_map_.erase(feat_map_cache_.back().first);
      delete feat_map_cache_.back().second;
...
void VIOManager::retrieveFromVisualSparseMap(
  ...
  for (auto &iter : sub_feat_map) {
    ...
    if (corre_voxel != feat_map.end()) {
      ...
      if (!voxel_in_fov) {
        DeleteKeyList.emplace_back(position);
      }
    }
  }
  ...
  if (raycast_en) {
    for (int i = 0; i < length; i++) {
      ...
      for (const auto &it : rays_with_sample_points[i]) {
        ...
        // sample_points_temp.emplace_back(sample_point_w);
        ...
        if (corre_feat_map != feat_map.end()) {
          ...
          for (int j = 0; j < voxel_num; j++) {
            ...
            // sub_map_ray.emplace_back(pt); // cloud_visual_sub_map
            // add_sample = true;
            ...
            if (new_frame_->cam_->isInFrame(pc.cast<int>(), border)) {
              ...
              // sub_map_ray_fov.emplace_back(pt);
          ...
          if (iter != plane_map.end()) {
            ...
            if (current_octo->plane_ptr_->is_plane_) {
              ...
              visual_submap->add_from_voxel_map.emplace_back(plane_center);
              break;
            }
          }
        }
      }
      // if(add_sample) sample_points.emplace_back(sample_points_temp);
    }
  }
  ...
  for (int i = 0; i < length; i++) {
    if (grid_num[i] == TYPE_MAP) {
      ...
      // visual_sub_map_cur.emplace_back(pt); // before
      ...
      visual_submap->voxel_points.emplace_back(pt);
      visual_submap->propa_errors.emplace_back(error);
      visual_submap->search_levels.emplace_back(search_level);
      visual_submap->errors.emplace_back(error);
      visual_submap->warp_patch.emplace_back(patch_wrap);
      visual_submap->inv_expo_list.emplace_back(ref_ftr->inv_expo_time_);
...
void VIOManager::retrieveFromVisualSparseMapLRU(
    ...
    const std::unordered_map<
        ...
        &plane_map) {
  if (visual_feat_map_.size() <= 0) return;
  ...
  for (auto &iter : sub_feat_map) {
    ...
    // double t4 = omp_get_wtime();
    auto corre_voxel = visual_feat_map_.find(position);
    // double t5 = omp_get_wtime();

    if (corre_voxel != visual_feat_map_.end()) {
      bool voxel_in_fov = false;
      std::vector<VisualPoint *> &voxel_points =
  ...
  if (raycast_en) {
    for (int i = 0; i < length; i++) {
      ...
      for (const auto &it : rays_with_sample_points[i]) {
        ...
        auto corre_feat_map = visual_feat_map_.find(sample_pos);
        if (corre_feat_map != visual_feat_map_.end()) {
          bool voxel_in_fov = false;
          ...
          if (iter != plane_map.end()) {
            VoxelOctoTree *current_octo;
            current_octo =
                iter->second->second->find_correspond(sample_point_w);
            if (current_octo->plane_ptr_->is_plane_) {
              pointWithVar plane_center;
              VoxelPlane &plane = *current_octo->plane_ptr_;
              plane_center.point_w = plane.center_;
...
void VIOManager::generateVisualMapPoints(cv::Mat img,
  ...
  for (int i = 0; i < length; i++) {
    if (grid_num[i] == TYPE_POINTCLOUD)  // && (scan_value[i]>=50))
    {
      ...
      add += 1;
      // map_cur_frame.emplace_back(pt_new);
    }
  }
        ...
        if (range_dis <= 3 * plane.radius_) {
          ...
          if (dis_to_plane_abs < 3 * sqrt(sigma_l)) {
            ...
            if (normal_update < 0.0001 && pt->obs_.size() > 10) {
              ...
              // visual_converged_point.emplace_back(pt);
            }
          }
        }
      }
...
void VIOManager::updateReferencePatchLRU(
  ...
  for (int i = 0; i < visual_submap->voxel_points.size(); i++) {
    ...
    if (iter != plane_map.end()) {
      VoxelOctoTree *current_octo;
      current_octo = iter->second->second->find_correspond(p_w);
      if (current_octo->plane_ptr_->is_plane_) {
        VoxelPlane &plane = *current_octo->plane_ptr_;
        float dis_to_plane = plane.normal_(0) * p_w(0) +
                             plane.normal_(1) * p_w(1) +
      ...
      for (int y = 0; y < patch_size; ++y) {
        ...
        for (int x = 0; x < patch_size; ++x)  //, ++patch_ptr)
        {
          ...
          if (px[0] < 0 || px[1] < 0 || px[0] >= img_cur.cols - 1 ||
            ...
            pixel_warp_vec.emplace_back(pixel_warp);
          }
        }
        pixel_warp_matrix.emplace_back(pixel_warp_vec);
      }
...
void VIOManager::updateFrameState(StatesGroup state) {
  ...
  Pcw = -Rci * Rwi.transpose() * Pwi + Pci;
  new_frame_->T_f_w_ =
      SE3(Eigen::Quaterniond(Rcw).normalized().toRotationMatrix(),
...
//   std::string image_path = std::string(ROOT_DIR) + "Log/Colmap/images/" +
//   cnt_str + ".png";
//   // std::cout<<"[ VIO ] dumpDataForColmap: " << image_path << std::endl;
...
//   fout_tum << cnt <<" "<< std::fixed << std::setprecision(6)  //
//   保证浮点数精度为6位
//             << t.x() << " " << t.y() << " " << t.z() << " "
//             << q.w() << " " << q.x() << " " << q.y() << " " << q.z() <<
//             std::endl;
//   cnt++;
// }
void VIOManager::dumpDataForColmap(double img_time) {
  static int cnt = 1;
  static Eigen::Quaterniond last_q = Eigen::Quaterniond::Identity();
  static Eigen::Vector3d last_t = Eigen::Vector3d::Zero();
  static const double rotation_threshold =
      0.1;  // Threshold for rotation in radians
  static const double translation_threshold =
      0.4;  // Threshold for translation in meters

  Eigen::Quaterniond q(new_frame_->T_f_w_.rotationMatrix());
  Eigen::Vector3d t = new_frame_->T_f_w_.translation();

  // Calculate rotation and translation differences
  double rotation_diff = q.angularDistance(last_q);
  double translation_diff = (t - last_t).norm();

  // 增加子地图保存功能

  // Check if the frame qualifies as a keyframe
  if (rotation_diff > rotation_threshold ||
      translation_diff > translation_threshold) {
    std::ostringstream ss, ss_t;
    ss << std::setw(5) << std::setfill('0') << cnt;
    std::string cnt_str = ss.str();
    ss_t << std::fixed << std::setprecision(4) << img_time * 1e3;
    std::string img_time = ss_t.str();
    ...
    fout_colmap << cnt << " " << std::fixed << std::setprecision(6) << q.w()
                << " " << q.x() << " " << q.y() << " " << q.z() << " " << t.x()
                << " " << t.y() << " " << t.z() << " " << 1
                << " "  // CAMERA_ID (assume camera ID is 1)
                << cnt_str << ".png" << std::endl;
    fout_colmap << "0.0 0.0 -1" << std::endl;

    // fout_tum << cnt_str<< " " << std::fixed << std::setprecision(6) << t.x()
    // << " "
    //          << t.y() << " " << t.z() << " " << q.x() << " "
    //          << q.y() << " " << q.z()<<" "<< q.w() << std::endl;
    fout_tum << img_time << " " << std::fixed << std::setprecision(6) << t.x()
             << " " << t.y() << " " << t.z() << " " << q.x() << " " << q.y()
             << " " << q.z() << " " << q.w() << std::endl;
...
void VIOManager::processFrame(
  ...
  retrieveFromVisualSparseMap(img, pg, lidar_feat_map);
  ...
  updateReferencePatch(lidar_feat_map);
...
void VIOManager::processFrame(
  ...
  std::cout << "[ VIO ] feat_map size: " << visual_feat_map_.size()
            << std::endl;
