...
bool pairIntAndStringSort(const std::pair<int, std::string>& pair_1,
                          const std::pair<int, std::string>& pair_2) {
  return pair_1.first < pair_2.first;
}

// initialize Session
MultiSession::Session::Session(int _idx, std::string _name,
                               std::string _session_dir_path,
                               bool _is_base_session)
    : index_(_idx),
      name_(_name),
      session_dir_path_(_session_dir_path),
      is_base_session_(_is_base_session) {
  allocateMemory();
...
// isam2 update
void MultiSession::Session::updateKeyPoses(
    const gtsam::ISAM2* _isam, const gtsam::Pose3& _anchor_transform) {
  gtsam::Values isamCurrentEstimate = _isam->calculateEstimate();

  int numPoses = cloudKeyFrames.size();
  for (int node_idx_in_sess = 0; node_idx_in_sess < numPoses;
       ++node_idx_in_sess) {
    int node_idx_in_global = genGlobalNodeIdx(index_, node_idx_in_sess);
    std::cout << "update the session " << index_
              << "'s node: " << node_idx_in_sess
              << " (global idx: " << node_idx_in_global << ")" << std::endl;

    gtsam::Pose3 pose_self_coord =
        isamCurrentEstimate.at<gtsam::Pose3>(node_idx_in_global);
    gtsam::Pose3 pose_central_coord = _anchor_transform * pose_self_coord;

    cloudKeyPoses6D->points[node_idx_in_sess].x =
        pose_central_coord.translation().x();
    cloudKeyPoses6D->points[node_idx_in_sess].y =
        pose_central_coord.translation().y();
    cloudKeyPoses6D->points[node_idx_in_sess].z =
        pose_central_coord.translation().z();
    cloudKeyPoses6D->points[node_idx_in_sess].roll =
        pose_central_coord.rotation().roll();
    cloudKeyPoses6D->points[node_idx_in_sess].pitch =
        pose_central_coord.rotation().pitch();
    cloudKeyPoses6D->points[node_idx_in_sess].yaw =
        pose_central_coord.rotation().yaw();
  }
}

void MultiSession::Session::loopFindNearKeyframesCentralCoord(
    KeyFrame& nearKeyframes, const int& key, const int& searchNum) {
  ...
  for (int i = -searchNum; i <= searchNum; ++i) {
    ...
    *nearKeyframes.all_cloud += *transformPointCloud(
        cloudKeyFrames[keyNear].all_cloud, &cloudKeyPoses6D->points[keyNear]);
  }

  if (nearKeyframes.all_cloud->empty()) return;

  // downsample near keyframes
  pcl::PointCloud<PointTypeXYZI>::Ptr cloud_temp(
...
void MultiSession::Session::loopFindNearKeyframesLocalCoord(
    KeyFrame& nearKeyframes, const int& key, const int& searchNum) {
  // extract near keyframes
  nearKeyframes.all_cloud->clear();
  int cloudSize = cloudKeyPoses6D->size();
  for (int i = -searchNum; i <= searchNum; ++i) {
    ...
    if (keyNear < 0 || keyNear >= cloudSize) continue;
    *nearKeyframes.all_cloud += *transformPointCloud(
        cloudKeyFrames[keyNear].all_cloud, &originPoses6D->points[0]);
  }
...
void MultiSession::Session::loadSessionKeyframePointclouds() {
  ...
  for (auto const& _pcd_name : pcd_names) {
    ...
    pcl::PointCloud<PointTypeXYZI>::Ptr thisCloudFrame(
        new pcl::PointCloud<PointTypeXYZI>());
  ...
  PCL_INFO_STREAM("\033[1;32m PCDs are loaded ("
                  << name_ << ") with size: " << cloudKeyFrames.size()
                  << "\033[0m\n");
}
...
void MultiSession::Session::loadSessionScanContextDescriptors() {
  ...
  PCL_INFO_STREAM("\033[1;32m SCDs are loaded ("
                  << name_ << ") with size: " << cloudKeyFrames.size()
                  << "\033[0m\n");
}
...
bool MultiSession::Session::loadSessionGraph() {
  ...
  while (getline(posefile_handle, strOneLine)) {
      ...
      Node this_node{
          ...
          gtsam::Pose3(
              gtsam::Rot3(gtsam::Quaternion(
                  line_info.quat[3], line_info.quat[0], line_info.quat[1],
                  line_info.quat[2])),  // xyzw to wxyz
              gtsam::Point3(line_info.trans[0], line_info.trans[1],
                            line_info.trans[2]))};
    ...
    if (isTwoStringSame(line_info.type, G2oLineInfo::kEdgeTypeName)) {
      Edge this_edge{
          line_info.prev_idx, line_info.curr_idx,
          gtsam::Pose3(
              gtsam::Rot3(gtsam::Quaternion(
                  line_info.quat[3], line_info.quat[0], line_info.quat[1],
                  line_info.quat[2])),  // xyzw to wxyz
...
void MultiSession::Session::loadGlobalMap() {
  ...
  globalMap->points.resize(raw_global_map->points.size());
#pragma omp parallel for
  for (size_t i = 0; i < raw_global_map->points.size(); ++i) {
    const auto& p = raw_global_map->points[i];
    PointTypeXYZI pt;
    pt.x = p.x;
    pt.y = p.y;
    pt.z = p.z;
    pt.intensity = p.intensity;
    globalMap->points[i] = pt;
  }

  PCL_INFO_STREAM("\033[1;32m Map loaded: " << mapfile_path << ", size: "
...
gtsam::Pose3 MultiSession::IncreMapping::getPoseOfIsamUsingKey(
    const gtsam::Key _key) {
  const gtsam::Value& pose_value = isam->calculateEstimate(_key);
  auto p_pose_value =
      dynamic_cast<const gtsam::GenericValue<gtsam::Pose3>*>(&pose_value);
  gtsam::Pose3 pose = gtsam::Pose3{p_pose_value->value()};
  return pose;
}

void MultiSession::IncreMapping::writeAllSessionsTrajectories(
    std::string _postfix = "") {
  // parse
  ...
  for (auto& _session_info : parsed_poses) {
    ...
    std::string filename_local = sessions_dir_ + save_directory_ +
                                 session_names[session_idx] + "_local_" +
                                 _postfix + ".txt";
    std::string filename_central = sessions_dir_ + save_directory_ +
                                   session_names[session_idx] + "_central_" +
                                   _postfix + ".txt";
    ...
    for (auto& _pose : _session_info.second) {
      ...
      gtsam::Pose3 pose_central =
          anchor_transform * _pose;  // se3 compose (oplus)
...
void MultiSession::IncreMapping::run(int iteration) {
  ...
  optimizeMultisesseionGraph(
      true, iteration);  // optimize the graph with existing edges
  writeAllSessionsTrajectories(std::string("bfr_intersession_loops"));

  std::cout << "----------  sc estimate -----------" << std::endl;
  detectInterSessionSCloops();  // detectInterSessionRSloops was internally done
                                // while sc detection
  addSCloops();
  optimizeMultisesseionGraph(
      true,
      iteration);  // optimize the graph with existing edges + SC loop edges

  std::cout << "----------  rs estimate -----------" << std::endl;
  bool toOpt =
      addRSloops();  // using the optimized estimates (rough alignment using SC)
  optimizeMultisesseionGraph(
      toOpt, iteration);  // optimize the graph with existing edges + SC loop
                          // edges + RS loop edges

  writeAllSessionsTrajectories(std::string("aft_intersession_loops"));

  std::string aftPose1 =
      sessions_dir_ + save_directory_ + "aft_tansformation1.pcd";
  pcl::io::savePCDFileASCII(aftPose1,
                            *sessions_.at(target_sess_idx).cloudKeyPoses6D);

  std::string aftPose2 =
      sessions_dir_ + save_directory_ + "aft_tansformation2.pcd";
  pcl::io::savePCDFileASCII(aftPose2,
                            *sessions_.at(source_sess_idx).cloudKeyPoses6D);
...
void MultiSession::IncreMapping::initNoiseConstants() {
  ...
  robustNoise = gtsam::noiseModel::Robust::Create(
      gtsam::noiseModel::mEstimator::Cauchy::Create(
          1),  // optional: replacing Cauchy by DCS or GemanMcClure, but with a
               // good front-end loop detector, Cauchy is empirically enough.
      gtsam::noiseModel::Diagonal::Variances(
          robustNoiseVector6));  // - checked it works. but with robust kernel,
                                 // map modification may be delayed (i.e,.
...
void MultiSession::IncreMapping::updateSessionsPoses() {
  for (auto& _sess_pair : sessions_) {
    auto& _sess = _sess_pair.second;
    gtsam::Pose3 anchor_transform =
        isamCurrentEstimate.at<gtsam::Pose3>(genAnchorNodeIdx(_sess.index_));
...
void MultiSession::IncreMapping::optimizeMultisesseionGraph(bool _toOpt,
  ...
  isamCurrentEstimate =
      isam->calculateEstimate();  // must be followed by update
  ...
  if (is_display_debug_msgs_) {
    std::cout << "***** variable values after optimization" << iteration
              << " *****" << std::endl;
    // std::cout << std::endl;
    // isamCurrentEstimate.print("Current estimate: ");
    // std::cout << std::endl;
    // std::ofstream
    // os("/home/user/Documents/catkin2021/catkin_ltmapper/catkin_ltmapper_dev/src/ltmapper/data/3d/kaist/PoseGraphExample.dot");
...
MultiSession::IncreMapping::doICPVirtualRelative(  // for SC loop
  ...
  pcl::PointCloud<PointTypeXYZI>::Ptr cureKeyframeCloud(
      new pcl::PointCloud<PointTypeXYZI>());
  pcl::PointCloud<PointTypeXYZI>::Ptr targetKeyframeCloud(
      new pcl::PointCloud<PointTypeXYZI>());
  ...
  source_sess.loopFindNearKeyframesLocalCoord(cureKeyframe,
                                              loop_idx_source_session, 0);
  target_sess.loopFindNearKeyframesLocalCoord(
      targetKeyframe, loop_idx_target_session, historyKeyframeSearchNum);
  mtx.unlock();  // unlock after loopFindNearKeyframesWithRespectTo because many
                 // new in the loopFindNearKeyframesWithRespectTo

  // ICP Settings
  pcl::IterativeClosestPoint<PointTypeXYZI, PointTypeXYZI> icp;
  icp.setMaxCorrespondenceDistance(
      30);  // giseop , use a value can cover 2*historyKeyframeSearchNum range
            // in meter
  ...
  if (icp.hasConverged() == false ||
    ...
    mtx.lock();
    std::cout << "  [SC loop] ICP fitness test failed ("
              << icp.getFitnessScore() << " > " << loopFitnessScoreThreshold
    ...
    reloKeyFrames.emplace_back(
        std::make_pair(loop_idx_source_session, keyframe));
    mtx.unlock();
  }
...
MultiSession::IncreMapping::doICPGlobalRelative(  // For RS loop
  ...
  source_sess.loopFindNearKeyframesCentralCoord(cureKeyframe,
                                                loop_idx_source_session, 0);
  target_sess.loopFindNearKeyframesCentralCoord(
      targetKeyframe, loop_idx_target_session, historyKeyframeSearchNum);
  mtx.unlock();  // unlock after loopFindNearKeyframesWithRespectTo because many
                 // new in the loopFindNearKeyframesWithRespectTo
  ...
  if (icp.hasConverged() == false ||
      icp.getFitnessScore() > loopFitnessScoreThreshold) {
    mtx.lock();
    std::cout << "  [RS loop] ICP fitness test failed ("
              << icp.getFitnessScore() << " > " << loopFitnessScoreThreshold
  ...
  Eigen::Affine3f tWrong = pclPointToAffine3f(
      source_sess.cloudKeyPoses6D->points[loop_idx_source_session]);
  ...
  gtsam::Pose3 poseTo = pclPointTogtsamPose3(
      target_sess.cloudKeyPoses6D->points[loop_idx_target_session]);

  return poseFrom.between(poseTo);
}  // doICPGlobalRelative

void MultiSession::IncreMapping::detectInterSessionSCloops()  // using
                                                              // ScanContext
{
  ...
  TicToc time_count;
  for (int source_node_idx = 0;
       source_node_idx < int(source_scManager.polarcontexts_.size());
       source_node_idx++) {
    std::vector<float> source_node_key =
        source_scManager.polarcontext_invkeys_mat_.at(source_node_idx);
    Eigen::MatrixXd source_node_scd =
        source_scManager.polarcontexts_.at(source_node_idx);

    auto detectResult = target_scManager.detectLoopClosureIDBetweenSession(
        source_node_key, source_node_scd);  // first: nn index, second: yaw diff

    int loop_idx_source_session = source_node_idx;
    int loop_idx_target_session = detectResult.first;

    if (loop_idx_target_session ==
        -1) {  // TODO using NO_LOOP_FOUND rather using -1
      RSLoopIdxPairs_.emplace_back(std::make_pair(
          -1,
          loop_idx_source_session));  // -1 will be later be found (nn pose).
      continue;
    }

    SCLoopIdxPairs_.emplace_back(
        std::make_pair(loop_idx_target_session, loop_idx_source_session));
  }
  double ave_time = time_count.toc() / SCLoopIdxPairs_.size();
  PCL_INFO_STREAM("\033[1;32m Total "
                  << SCLoopIdxPairs_.size()
                  << " inter-session loops are found. Average time " << ave_time
                  << " \033[0m");
}  // detectInterSessionSCloops

void MultiSession::IncreMapping::detectInterSessionRSloops()  // using
                                                              // ScanContext
...
std::vector<std::pair<int, int>> MultiSession::IncreMapping::equisampleElements(
    const std::vector<std::pair<int, int>>& _input_pair, float _gap,
    int _num_sampled) {
...
void MultiSession::IncreMapping::addSCloops() {
  if (SCLoopIdxPairs_.empty()) return;
  ...
  auto sc_loop_idx_pairs_sampled = equisampleElements(
      SCLoopIdxPairs_, equisampling_gap, num_scloops_to_be_added);
  ...
  for (int ith = 0; ith < num_scloops_sampled; ith++) {
    ...
    auto relative_pose_optional =
        doICPVirtualRelative(target_sess, source_sess, loop_idx_target_session,
                             loop_idx_source_session);

    if (relative_pose_optional) {
      mtx.lock();
      gtsam::Pose3 relative_pose = relative_pose_optional.value();
      gtSAMgraph.add(gtsam::BetweenFactorWithAnchoring<gtsam::Pose3>(
          genGlobalNodeIdx(target_sess_idx, loop_idx_target_session),
          genGlobalNodeIdx(source_sess_idx, loop_idx_source_session),
      ...
      cout << "SCdetector found an inter-session edge between "
           ...
           << " and "
           << genGlobalNodeIdx(source_sess_idx, loop_idx_source_session)
           << " (anchor nodes are " << genAnchorNodeIdx(target_sess_idx)
           << " and " << genAnchorNodeIdx(source_sess_idx) << ")" << endl;
      mtx.unlock();
    }
  }
}  // addSCloops

double MultiSession::IncreMapping::calcInformationGainBtnTwoNodes(
    const int loop_idx_target_session, const int loop_idx_source_session) {
  auto pose_s1 = isamCurrentEstimate.at<gtsam::Pose3>(genGlobalNodeIdx(
      target_sess_idx, loop_idx_target_session));  // node: s1 is the central
  auto pose_s2 = isamCurrentEstimate.at<gtsam::Pose3>(
      genGlobalNodeIdx(source_sess_idx, loop_idx_source_session));
  auto pose_s1_anchor =
      isamCurrentEstimate.at<gtsam::Pose3>(genAnchorNodeIdx(target_sess_idx));
  auto pose_s2_anchor =
      isamCurrentEstimate.at<gtsam::Pose3>(genAnchorNodeIdx(source_sess_idx));

  gtsam::Pose3 hx1 = gtsam::traits<gtsam::Pose3>::Compose(
      pose_s1_anchor, pose_s1);  // for the updated jacobian, see line 60, 219,
                                 // https://gtsam.org/doxygen/a00053_source.html
  gtsam::Pose3 hx2 =
      gtsam::traits<gtsam::Pose3>::Compose(pose_s2_anchor, pose_s2);
  gtsam::Pose3 estimated_relative_pose =
      gtsam::traits<gtsam::Pose3>::Between(hx1, hx2);
  ...
  gtsam::Matrix pose_s1_cov = isam->marginalCovariance(genGlobalNodeIdx(
      target_sess_idx,
      loop_idx_target_session));  // note: typedef Eigen::MatrixXd gtsam::Matrix
  gtsam::Matrix pose_s2_cov = isam->marginalCovariance(
      genGlobalNodeIdx(source_sess_idx, loop_idx_source_session));

  // calc S and information gain
  gtsam::Matrix Sy =
      Eigen::MatrixXd::Identity(6, 6);  // measurement noise, assume fixed
  gtsam::Matrix S = Sy + (H_s1 * pose_s1_cov * H_s1.transpose() +
                          H_s2 * pose_s2_cov * H_s2.transpose());
...
void MultiSession::IncreMapping::
    findNearestRSLoopsTargetNodeIdx()  // based-on information gain
{
  ...
  for (std::size_t i = 0; i < RSLoopIdxPairs_.size(); i++) {
    ...
    auto rsloop_global_idx_source_session =
        genGlobalNodeIdx(source_sess_idx, rsloop_idx_source_session);

    auto source_node_idx = rsloop_idx_source_session;
    auto query_pose =
        isamCurrentEstimate.at<gtsam::Pose3>(rsloop_global_idx_source_session);
    gtsam::Pose3 query_sess_anchor_transform =
        isamCurrentEstimate.at<gtsam::Pose3>(genAnchorNodeIdx(source_sess_idx));
    ...
    for (int target_node_idx = 0;
         target_node_idx < int(target_sess.nodes_.size()); target_node_idx++) {
      auto target_pose = isamCurrentEstimate.at<gtsam::Pose3>(
          genGlobalNodeIdx(target_sess_idx, target_node_idx));
      if (poseDistance(query_pose_central_coord, target_pose) <
          10.0)  // 10 is a hard-coding for fast test
      {
        target_node_idxes_within_ball.push_back(target_node_idx);
        // cout << "(all) RS pair detected: " << target_node_idx << " <-> " <<
        // source_node_idx << endl;
      }
    }

    // if no nearest one, skip
    if (target_node_idxes_within_ball.empty()) continue;
    ...
    for (int i = 0; i < target_node_idxes_within_ball.size(); i++) {
      ...
      double this_information_gain =
          calcInformationGainBtnTwoNodes(nn_target_node_idx, source_node_idx);
    ...
    // cout << "RS pair detected: " << selected_near_target_node_idx << " <-> "
    // << source_node_idx << endl; cout << "info gain: " << max_information_gain
    // << endl;

    validRSLoopIdxPairs.emplace_back(
        std::pair<int, int>{selected_near_target_node_idx, source_node_idx});
  }

  // update
  RSLoopIdxPairs_.clear();
  RSLoopIdxPairs_.resize((int)(validRSLoopIdxPairs.size()));
  std::copy(validRSLoopIdxPairs.begin(), validRSLoopIdxPairs.end(),
            RSLoopIdxPairs_.begin());
}
...
bool MultiSession::IncreMapping::addRSloops() {
  ...
  if (num_rsloops_all_found == 0) return false;

  int num_rsloops_to_be_added = num_rsloops_all_found;
  int equisampling_gap = num_rsloops_all_found / num_rsloops_to_be_added;

  auto rs_loop_idx_pairs_sampled = equisampleElements(
      RSLoopIdxPairs_, equisampling_gap, num_rsloops_to_be_added);
...
#pragma omp parallel for num_threads(numberOfCores)
  for (int ith = 0; ith < num_rsloops_sampled; ith++) {
    ...
    auto relative_pose_optional =
        doICPGlobalRelative(target_sess, source_sess, loop_idx_target_session,
                            loop_idx_source_session);
...
void MultiSession::IncreMapping::initTrajectoryByAnchoring(
    const Session& _sess) {
  ...
  if (_sess.is_base_session_) {
    gtSAMgraph.add(gtsam::PriorFactor<gtsam::Pose3>(
        this_session_anchor_node_idx, poseOrigin, priorNoise));
  } else {
    gtSAMgraph.add(gtsam::PriorFactor<gtsam::Pose3>(
        this_session_anchor_node_idx, poseOrigin, largeNoise));
  }

  initialEstimate.insert(this_session_anchor_node_idx, poseOrigin);
}  // initTrajectoryByAnchoring

void MultiSession::IncreMapping::addSessionToCentralGraph(
    const Session& _sess) {
  // add nodes
  for (auto& _node : _sess.nodes_) {
    ...
    gtsam::Vector Vector6(6);
    if (node_idx == 0) {  // TODO consider later if the initial node idx is not
                          // zero (but if using SC-LIO-SAM, don't care)
      // prior node
      gtSAMgraph.add(gtsam::PriorFactor<gtsam::Pose3>(curr_node_global_idx,
                                                      curr_pose, priorNoise));
  ...
  for (auto& _edge : _sess.edges_) {
    ...
    if (std::abs(to_node_idx - from_node_idx) == 1) {
      ...
      gtSAMgraph.add(gtsam::BetweenFactor<gtsam::Pose3>(
          from_node_global_idx, to_node_global_idx, relative_pose, odomNoise));
      if (is_display_debug_msgs_)
        cout << "add an odom edge between " << from_node_global_idx << " and "
             << to_node_global_idx << endl;
    } else {
      // loop edge
      gtSAMgraph.add(gtsam::BetweenFactor<gtsam::Pose3>(
          from_node_global_idx, to_node_global_idx, relative_pose,
          robustNoise));
      if (is_display_debug_msgs_)
        cout << "add a loop edge between " << from_node_global_idx << " and "
             << to_node_global_idx << endl;
    }
  }
}
...
void MultiSession::IncreMapping::loadAllSessions() {
  // pose
  PCL_INFO_STREAM("\033[1;32m Load sessions' pose data from: " << sessions_dir_
                                                               << "\033[0m");
  for (auto& _session_entry : fs::directory_iterator(sessions_dir_)) {
    ...
    if (!isTwoStringSame(session_name, central_sess_name_) &
        !isTwoStringSame(session_name, query_sess_name_)) {
      continue;  // jan. 2021. currently designed for two-session ver. (TODO: be
                 // generalized for N-session co-optimization)
    }
    ...
    // sessions_.emplace_back(Session(session_idx, session_name,
    // session_dir_path, isTwoStringSame(session_name, central_sess_name_)));
    sessions_.insert(std::make_pair(
        session_idx,
        Session(session_idx, session_name, session_dir_path,
                isTwoStringSame(session_name, central_sess_name_))));

    // MultiSession::IncreMapping::num_sessions++; // incr the global index //
    // TODO: make this private and provide incrSessionIdx
  }

  std::cout << std::boolalpha;
  PCL_INFO_STREAM("\033[1;32m Total : " << sessions_.size()
                                        << " sessions are loaded.\033[0m");
  std::for_each(sessions_.begin(), sessions_.end(), [](auto& _sess_pair) {
    cout << " — " << _sess_pair.second.name_
         << " (is central: " << _sess_pair.second.is_base_session_ << ")"
         << endl;
  });
}  // loadSession
...
void MultiSession::IncreMapping::visualizeLoopClosure() {
  ...
  if (SCLoopIdxPairs_.empty() && RSLoopIdxPairs_.empty()) return;

  // show sc
  visualization_msgs::msg::MarkerArray markerArray_sc;
  // 回环顶点
  visualization_msgs::msg::Marker markerNode_sc;
  ...
  markerNode_sc.action = visualization_msgs::msg::Marker::ADD;
  // 设置形状:球体
  markerNode_sc.type = visualization_msgs::msg::Marker::SPHERE_LIST;
  ...
  // 回环边
  visualization_msgs::msg::Marker markerEdge_sc;
  markerEdge_sc.header.frame_id = odometryFrame;
  // markerEdge_sc.header.stamp = timeLaserInfoStamp;
  markerEdge_sc.action = visualization_msgs::msg::Marker::ADD;
  // 设置形状:线
  markerEdge_sc.type = visualization_msgs::msg::Marker::LINE_LIST;
  ...
  // show rs
  visualization_msgs::msg::MarkerArray markerArray_rs;
  // rs回环顶点
  visualization_msgs::msg::Marker markerNode_rs;
  markerNode_rs.header.frame_id = odometryFrame;
  // markerNode_rs.header.stamp = timeLaserInfoStamp;
  // action对应的操作:ADD=0、MODIFY=0、DELETE=2、DELETEALL=3,即添加、修改、删除、全部删除
  markerNode_rs.action = visualization_msgs::msg::Marker::ADD;
  // 设置形状:球体
  markerNode_rs.type = visualization_msgs::msg::Marker::SPHERE_LIST;
  ...
  // 回环边
  visualization_msgs::msg::Marker markerEdge_rs;
  markerEdge_rs.header.frame_id = odometryFrame;
  // markerEdge_rs.header.stamp = timeLaserInfoStamp;
  markerEdge_rs.action = visualization_msgs::msg::Marker::ADD;
  // 设置形状:线
  markerEdge_rs.type = visualization_msgs::msg::Marker::LINE_LIST;
...
void MultiSession::IncreMapping::loadCentralMap() {
  ...
  std::cout << "load "
            << (sessions_dir_ + central_sess_name_ + "/globalMap.pcd")
            << " size: " << map->points.size() << std::endl;
}

// void MultiSession::IncreMapping::publish(){
//     publishCloud(&pubCentralGlobalMap, centralMap_, publishTimeStamp,
//     "camera_init");

//     publishCloud(&pubCentralTrajectory, traj_central, publishTimeStamp,
//     "camera_init");

//     publishCloud(&pubRegisteredTrajectory, traj_regis, publishTimeStamp,
//     "camera_init");
...
void MultiSession::IncreMapping::getReloKeyFrames() {
  for (auto& it : reloKeyFrames) {
    it.second.all_cloud = transformPointCloud(
        sessions_.at(source_sess_idx).cloudKeyFrames[it.first].all_cloud,
        &sessions_.at(source_sess_idx).cloudKeyPoses6D->points[it.first]);
    it.second.scv_od =
        sessions_.at(source_sess_idx).cloudKeyFrames[it.first].scv_od;
    *regisMap_ += *it.second.all_cloud;
  }

  for (int i = 0;
       i < sessions_.at(target_sess_idx).cloudKeyPoses6D->points.size(); i++) {
    PointTypeXYZI pt;
  ...
  for (int i = 0;
       i < sessions_.at(source_sess_idx).cloudKeyPoses6D->points.size(); i++) {
    PointTypeXYZI pt;
