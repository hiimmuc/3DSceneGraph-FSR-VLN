...
using PointTypeXYZI =
    pcl::PointXYZINormal;  // using xyzinormal for fast-lio. but a user can
...
class SCManager {
 public:
  SCManager() =
      default;  // reserving data space (of std::vector) could be considered.
                // but the descriptor is lightweight so don't care.

  Eigen::MatrixXd makeScancontext(pcl::PointCloud<PointTypeXYZI> &_scan_down);
  ...
  // User-side API
  void makeAndSaveScancontextAndKeys(
      pcl::PointCloud<PointTypeXYZI> &_scan_down);
  std::pair<int, float> detectClosestKeyframeID(
      int num_exclude_recent, const std::vector<float> &curr_key,
      Eigen::MatrixXd &curr_desc);
  std::pair<int, float> detectClosestKeyframeID(
      int num_exclude_recent, std::vector<int> &invalid_id,
      const std::vector<float> &curr_key, Eigen::MatrixXd &curr_desc);
  std::pair<int, float> detectLoopClosureID(
      int num_exclude_recent =
          50);  // int: nearest node index, float: relative yaw

  void saveCurrentSCD(const std::string &fileName, int num_digits = 6,
                      const std::string &delimiter = " ");
  void loadPriorSCD(const std::string &path, int num_digits, int num_keyframe);
  std::pair<int, float> relocalize(pcl::PointCloud<PointTypeXYZI> &_scan_down);
  ...
  std::pair<int, float> detectLoopClosureIDBetweenSession(
      std::vector<float> &curr_key, Eigen::MatrixXd &curr_desc);
 ...
 public:
  // hyper parameters ()
  double LIDAR_HEIGHT =
      0.0;  // lidar height : add this for simply directly using lidar scan in
  ...
  const int NUM_EXCLUDE_RECENT =
      30;  // simply just keyframe gap (related with loopClosureFrequency in
           // yaml), but node position distance-based exclusion is ok.
  const int NUM_CANDIDATES_FROM_TREE =
      8;  // 10 is enough. (refer the IROS 18 paper)
  ...
  // const double SC_DIST_THRES = 0.13; // empirically 0.1-0.2 is fine (rare
  // false-alarms) for 20x60 polar context (but for 0.15 <, DCS or ICP fit score
  // check (e.g., in LeGO-LOAM) should be required for robustness)
  double SC_DIST_THRES =
      0.3;  // 0.4-0.6 is good choice for using with robust kernel (e.g.,
            // Cauchy, DCS) + icp fitness threshold / if not, recommend 0.1-0.15

  // config
  const int TREE_MAKING_PERIOD_ =
      10;  // i.e., remaking tree frequency, to avoid non-mandatory every
           // remaking, to save time cost / if you want to find a very recent
           // revisits use small value of it (it is enough fast ~ 5-50ms wrt
           // N.).
