...
template <typename PointTarget, typename PointSource>
double LsqRegistration<PointTarget, PointSource>::evaluateCost(
    const Eigen::Matrix4f& relative_pose, Eigen::Matrix<double, 6, 6>* H,
...
bool LsqRegistration<PointTarget, PointSource>::step_optimize(
    Eigen::Isometry3d& x0, Eigen::Isometry3d& delta) {
...
bool LsqRegistration<PointTarget, PointSource>::step_gn(
  ...
  // 用 step_size_ 缩放步长
  Eigen::Matrix<double, 6, 1> d_scaled = step_size_ * d;
  delta = se3_exp(d_scaled);
...
bool LsqRegistration<PointTarget, PointSource>::step_lm(
  ...
  for (int i = 0; i < lm_max_iterations_; i++) {
    ...
    if (lm_debug_print_) {
      ...
      char dec = rho > 0.0 ? 'x' : ' ';
      std::cout << boost::format("%5d %15g %15g %15g %15g %15g %5c") % i % y0 %
                       yi % rho % lm_lambda_ % d.norm() % dec
                << std::endl;
    }
...
// 在实现文件中
template <typename PointTarget, typename PointSource>
void LsqRegistration<PointTarget, PointSource>::setStepSize(double step_size) {
