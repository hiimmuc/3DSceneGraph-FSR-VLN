...
def main(params: DictConfig):
    ...
    for ply_path, info_path in zip(objects_ply_paths, objects_info_paths):
        ...
        hier_topo[room_infos[object_info["room_id"]]["floor_id"]][
            room_infos[object_info["room_id"]]["room_id"]
        ].append(object_info["object_id"])
    ...
    root_node_centroid_viz = (
        max_floor_centroid + floor_infos[max_floor_id]["viz_offset"] + root_offset
    )
    ...
    for obj_id, obj_info in object_infos.items():
        if (
            ...
            p.add_mesh(
                pv.Line(
                    tuple(room_centroids_viz[obj_info["room_id"]]),
                    tuple(obj_centroids_viz[obj_id]),
                ),
                line_width=1.5,
                opacity=0.5,
            )
            ...
            center = cloud_xyz.mean(
                axis=0
            )  # Calculate the centroid of the current furniture cluster
            cloud = pv.PolyData(cloud_xyz)
            p.add_mesh(
                cloud,
            ...
            p.add_point_labels(
                [center], [object_name], font_size=8, point_color="blue", text_color="black"
            )
