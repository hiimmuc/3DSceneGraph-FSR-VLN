...
class VisualizationConfig:
    ...
    excluded_object_names: Tuple[str, ...] = (
        ...
        "stair", "window", "glass", "railing", "glass doors",
        "whiteboard", "sliding door", "carpet", "picture"
    )
...
def _load_rooms(graph_path: str, floor_infos: Dict, config: VisualizationConfig) -> Tuple[Dict, Dict]:
...
def _should_include_object(obj_info: Dict, obj_pcd: o3d.geometry.PointCloud,
...
def _visualize_hierarchy(plotter: pv.Plotter, floor_pcds: Dict, floor_infos: Dict,
                        room_pcds: Dict, room_infos: Dict, object_pcds: Dict,
                        object_infos: Dict, hier_topo: Dict, config: VisualizationConfig) -> None:
    """Add hierarchy visualization to plotter."""
    # Visualize floor centroids
    floor_centroids = {
        fid: np.mean(np.asarray(floor_pcds[fid].points), axis=0)
    ...
    for fid, centroid in floor_centroids_viz.items():
        plotter.add_mesh(
            pv.Sphere(center=tuple(centroid), radius=config.sphere_radius_floor),
            color="orange"
        )
    
    # Visualize room centroids
    room_centroids = {
        rid: np.mean(np.asarray(room_pcds[rid].points), axis=0)
    ...
    for rid, centroid in room_centroids_viz.items():
        plotter.add_mesh(
            pv.Sphere(center=tuple(centroid), radius=config.sphere_radius_room),
            color="blue"
        )
    ...
    for obj_id, obj_info in object_infos.items():
        ...
        plotter.add_point_labels(
            [obj_centroid], [obj_info["name"]],
            font_size=8, point_color="blue", text_color="black"
        )
...
def main(params: DictConfig) -> None:
    ...
    parser.add_argument("--min-object-points", type=int, default=None, help="Minimum points for object inclusion")
    ...
    _visualize_hierarchy(plotter, floor_pcds, floor_infos, room_pcds,
                        room_infos, object_pcds, object_infos, hier_topo, config)
