...
def _colorize_clusters(pcd: o3d.geometry.PointCloud, labels: np.ndarray,
                       cmap_name: str = "tab20") -> None:
...
def _get_cluster_names(labels: np.ndarray,
                       provided_names: Optional[Dict[int, str]] = None) -> Dict[int, str]:
...
def _setup_scene(window: gui.Window, pcd: o3d.geometry.PointCloud,
                 labels: np.ndarray, cluster_names: Dict[int, str],
                 config: VisualizationConfig) -> gui.SceneWidget:
...
def visualize_point_cloud_with_labels(pcd: o3d.geometry.PointCloud,
                                      labels: np.ndarray,
                                      cluster_names: Optional[Dict[int, str]] = None,
    ...
    window = app.create_window(
        vis_config.window_title,
        vis_config.window_width,
        vis_config.window_height
    )
...
def _merge_cli_args(clustering_config: ClusteringConfig,
                   vis_config: VisualizationConfig,
                   cli_args: argparse.Namespace) -> Tuple[ClusteringConfig, VisualizationConfig]:
...
def main() -> None:
    ...
    parser.add_argument(
        "--pcd-path",
        type=str,
        default="/mnt/disk2/hovsg/HOV-SG/data/scannet/scene_graph/scannet/scene0378_00/full_pcd.ply",
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="DBSCAN epsilon (clustering radius)"
    )
    parser.add_argument(
        "--min-points",
        type=int,
        default=None,
        help="Minimum points for cluster (DBSCAN)"
    )
    parser.add_argument(
        "--cmap",
        type=str,
    ...
    clustering_config, vis_config = _merge_cli_args(
        clustering_config, vis_config, args
    )
