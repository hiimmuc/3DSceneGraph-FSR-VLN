...
def filter_masks(masks):
    """Check if any mask contain another mask and remove the smaller one frm the

    bigger one.
    return the filtered masks.

    Args:
        masks: (list): list of masks output from sam.
...
def show_anns(anns):
    ...
    Args:
        anns: (list): list of masks output from sam."""
    if len(anns) == 0:
        return
...
def increase_bbox_by_margin(bbox, margin):
    ...
    Args:
        bbox: The bounding box coordinates in XYWH format as a tuple of (x,
    y, w, h).
        margin: The margin to increase the bounding box size by in pixels.

    Returns:
        The increased bounding box coordinates as a tuple of (x, y, w, h)."""
    x, y, w, h = bbox
    x -= margin
    y -= margin
    w += margin * 2
...
def draw_all_bounding_boxs(
    ...
    Args:
        ...
        masks: all masks sam output.
        bbox: The bounding box coordinates in XYWH format as a tuple of (x,
    y, w, h).
        color: The color of the bounding box as a tuple of (R, G, B).
        thickness: The thickness of the bounding box edges in pixels.
        bbox_margin: increase the bbox by this value of pixel in all
    direction.
...
def crop_image(image, mask):
    ...
    Args:
        ...
        mask: (dict): The mask as a dictionary with the "segmentation" and
    "bbox" keys.
...
def crop_bbox(image, bbox, bbox_margin=0):
    ...
    Args:
        image: (np.array): The input image as a numpy array.
        bbox: (tuple): The bounding box coordinates in XYWH format as a tuple
    of (x, y, w, h).
        bbox_margin: (int): The margin to increase the bounding box size by
    in pixels.
...
def overlay_mask_on_image(image, mask, negative=False):
    ...
    Args:
        image: The input image as a numpy array.
        mask: The binary mask as a numpy array of the same size as the input
    image.

    Returns:
        The image with the mask overlayed on it."""
    output = image.copy()
    mask = np.array(mask)
...
def mask_and_crop_image(image, mask, bbox, bbox_margin=0):
    """Masks the input image with the provided binary mask and crops it to the

    bounding box.

    Args:
        image: (np.array): The input image as a numpy array.
        mask: (np.array): The binary mask as a numpy array of the same size
    as the input image.
        bbox: (tuple): The bounding box coordinates in XYWH format as a tuple
    of (x, y, w, h).
        bbox_margin: (int): The margin to increase the bounding box size by
    ...
    Returns:
        The cropped image with the mask overlayed on it."""
