...
def increase_bbox_by_margin(bbox, margin):
    ...
    :param bbox: The bounding box coordinates in XYWH format as a tuple of (x,
        y, w, h).
...
def draw_all_bounding_boxs(
    ...
    :param bbox_margin: increase the bbox by this value of pixel in all
        direction.
...
def crop_image(image, mask):
    ...
    :param mask (dict): The mask as a dictionary with the "segmentation" and
        "bbox" keys.
...
def crop_bbox(image, bbox, bbox_margin=0):
    ...
    :param image (np.array): The input image as a numpy array.
    :param bbox (tuple): The bounding box coordinates in XYWH format as a tuple
        of (x, y, w, h).
    :param bbox_margin (int): The margin to increase the bounding box size by
        in pixels.
    :return: The cropped image.
    """
    # Add the margin to the bounding box
...
def overlay_mask_on_image(image, mask, negative=False):
    ...
    :param image: The input image as a numpy array.
    :param mask: The binary mask as a numpy array of the same size as the input
        image.
...
def mask_and_crop_image(image, mask, bbox, bbox_margin=0):
    ...
    :param image (np.array): The input image as a numpy array.
    :param mask (np.array): The binary mask as a numpy array of the same size
        as the input image.
    :param bbox (tuple): The bounding box coordinates in XYWH format as a tuple
        of (x, y, w, h).
    :param bbox_margin (int): The margin to increase the bounding box size by
