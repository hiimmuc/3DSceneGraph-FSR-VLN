...
# Function to read PLY file and assign colors based on object_id for
# replica dataset
def read_ply_and_assign_colors_replica(file_path, semantic_info_path):
    ...
    :param file_path: path to PLY file :param semantic_info_path: path to
    semantic info JSON file :return: point cloud, class ids, point cloud
    instance, object ids."""
    # Read PLY file
...
def text_prompt(clip_model, clip_feat_dim, mask_feats, text, templates=True):
    """Compute similarity between text and mask_feats :param clip_model: CLIP
    model :param clip_feat_dim: CLIP feature dimension :param mask_feats: mask
    features :param text: text :param templates: whether to use templates
    :return: similarity."""
    text_list = text
    if templates:
...
def read_semantic_classes_replica(semantic_info_path, crete_color_map=False):
    """
    Read semantic classes for replica dataset :param semantic_info_path:

    path to semantic info JSON file
...
def load_feature_map(path, normalize=True):
    """Load features map from disk, mask_feats.pt and objects/pcd_i.ply :param
    path: path to feature map :param normalize: whether to normalize features
...
def sim_2_label(similarity, labels_id):
    """Convert similarity matrix to labels :param similarity: similarity matrix
    :param labels_id: labels id :return: labels."""
...
def Tree_interpolation(pred_pc: np.ndarray, gt_pc: np.ndarray):
    """Using cKdtree assign create new pred_label based on gt_label for each
    point in pred_pc, find the nearest point in gt_pc and assign its label."""
...
def knn_interpolation(cumulated_pc: np.ndarray, full_sized_data: np.ndarray, k):
    ...
    k nearest neighbor interpolation :return: pointcloud with predicted labels
    in last column and ground truth labels in last but one column."""
...
def read_ply_and_assign_colors(file_path, semantic_info_path):
    """
    Read PLY file and assign colors based on object_id :param file_path:
