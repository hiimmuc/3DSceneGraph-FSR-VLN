...
def generate_long_queries(tree: nx.DiGraph) -> Tuple[List[str], List[Tuple]]:
    """
    Generate long hierarchical queries from the given tree. The queries are in
    the form of <object X in region Y on floor Z>.
...
def generate_gt_object_nodes(
    ...
    """
    The GT object nodes ids. Given a hierarchical query "X object in region Y
    on floor Z", there might be multiple correct target objects. For example,
    there are multiple "region Y" on the floor, and "object X" exists in
    several "region Y". This function computes all valid object instances given
    a certain query.
...
def generate_long_query_dataset_hm3dsem(
    ...
    for scene_dir in scene_dirs:
        ...
        # Generate GT text queries and accompanying text queries (floor_id,
        # room_id, object_id)
        queries = []
        gt_node_tuples = []
        for gt_obj_node in evaluator.gt_objects:
            ...
            # Depending on the query spec, define the GT tuple that is asked
            # for given the query
