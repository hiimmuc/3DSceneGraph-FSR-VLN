...
class SimplePCDProjection:
    ...
    def load_pcd_with_intensity(self, pcd_file):
        ...
        # points, intensities = self.voxel_downsample(points, None, voxel_size=0.005)
        
        return points, intensities

    def voxel_downsample(self, points, intensities, voxel_size=0.002):
        if not O3D_AVAILABLE:
            self.log("  ⚠ Open3D not available, cannot perform voxel downsampling")
            return points, intensities
        ...
        # 根据索引获取对应的强度值
        down_intensities = intensities[indices]
        
        ratio = len(down_points) / len(points) if len(points) > 0 else 0
        self.log(f"  ✓ Voxel downsampling complete: {len(points)} → {len(down_points)} points ({ratio:.2%} remaining)")
    ...
    def process_bk(self):
        ...
        try:
            ...
            # 获取数据
            T_w_cam = self.camera_poses[cam_id][frame_id]['T_w_cam']
            ...
            self.log(f"  图像: {image.shape[1]}x{image.shape[0]}")
            
            # 投影
            self.log("  投影点云...")
            points_image, points_camera, pixel_coords, depths, intensities = self.project_points(
                self.pcd_points, self.pcd_intensities, T_w_cam, params, image.shape, cam_id
            )

            # Tcw = np.linalg.inv(T_w_cam)
            print("points_image.shape:", points_image.shape)
            print("points_camera.shape:", points_camera.shape)

            
            self.log(f"  ✓ 投影 {len(pixel_coords)} 个点")

            # Deocc去遮挡
            img_input = image
            img_height, img_width, _ = img_input.shape
            print("image.shape: ", image.shape)
            if img_input is None:
                self.log(f"Failed to read RGB image: {img_path}")
                return

            dilate_view_path = os.path.join(DEFAULT_OUTPUT_DIR, f"{cam_id}_{frame_id}_dilate.png")
            depth_output_path = os.path.join(DEFAULT_OUTPUT_DIR, f"{cam_id}_{frame_id}.png")
            visualization_output_path = os.path.join(DEFAULT_OUTPUT_DIR, f"{cam_id}_{frame_id}_overlay.png")
            self.generate_occ_depth(points_image, points_camera, img_width, img_height, dilate_view_path, depth_output_path, visualization_output_path,
                            depth_factor=1000, img_input=img_input, image_scale=1)

            
            # 叠加
            self.log("  生成叠加图像...")
            result = self.overlay_points(image, pixel_coords, depths, intensities)
            ...
            self.log(f"  ✓ 保存: {output_name}")
            self.log("="*60)
            self.log("✓ 完成！")
            self.status_var.set("完成")
            
            messagebox.showinfo("成功", f"投影完成！\n{output_name}")
    ...
    def process(self):
        ...
        try:
            ...
            img_path = self.img_entry.get()
            # cam_id = os.path.dirname(img_path)
            basename = os.path.basename(img_path)
            ...
            for frame_id in range(0, len(os.listdir(img_dir))):
                ...
                # self.depth_dir = "/mnt/disk1/mapvln/pocket2/MT20251211-195452/image/depth"
                # self.depth_vis_dir = "/mnt/disk1/mapvln/pocket2/MT20251211-195452/image/depth_vis"
                # self.depth_dir = "/mnt/disk1/mapvln/pocket2/MT20251211-195707/image/depth"
                # self.depth_vis_dir = "/mnt/disk1/mapvln/pocket2/MT20251211-195707/image/depth_vis"
                self.depth_dir = f"/mnt/disk1/mapvln/pocket2/MT20260309-104919/image/rectify_depth_sequence/{cam_id}"
                self.depth_vis_dir = f"/mnt/disk1/mapvln/pocket2/MT20260309-104919/image/rectify_depth_vis_sequence/{cam_id}"
                ...
                dilate_view_path = os.path.join(self.depth_vis_dir, f"{frame_id:06d}_dilate.png")
                depth_output_path = os.path.join(self.depth_dir, f"{frame_id:06d}.png")
                visualization_output_path = os.path.join(self.depth_vis_dir, f"{frame_id:06d}_overlay.png")
                ...
                self.log(f"每个帧处理时间: {(end_time - start_time) / len(os.listdir(img_dir)):.4f} 秒")
...
#  [  0.       0.       1.    ]]
        ...
        K_dist = np.loadtxt(os.path.join(self.image_root_dir, f"rectify_{cam_id}_param.txt"))
        fx = K_dist[0, 0]
        fy = K_dist[1, 1]
        cx = K_dist[0, 2]
        cy = K_dist[1, 2]
